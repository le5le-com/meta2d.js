...
export function fan(pen: any) {
  ...
  for (let i = 0; i < 6; i++) {
    fanblade(pen, value + (Math.PI / 3) * i, myw, myh, x, y);
  }
  fanheart(pen, myw, myh, x, y);
  fanring(pen, myw, myh, x, y);
...
//扇心
function fanheart(pen: TopologyPen, myw: any, myh: any, x: any, y: any) {
  let r1 =
    pen.calculative.worldRect.width > pen.calculative.worldRect.height
      ? myh / 2
      : myw / 2;
...
//扇环
function fanring(pen: TopologyPen, myw: any, myh: any, x: any, y: any) {
  let r1 =
    pen.calculative.worldRect.width > pen.calculative.worldRect.height
      ? pen.calculative.worldRect.height / 2 - myh / 4
      : pen.calculative.worldRect.width / 2 - myw / 4;
