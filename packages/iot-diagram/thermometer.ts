...
export function thermometerDrawScaleByCtx(ctx: CanvasRenderingContext2D, pen: any) {
  let R: number, r: number;
...
function fillValuethermometer(path: Path2D, pen: any, R: number, r: number, thermometer: any) {
  ...
  path.moveTo(pen.calculative.worldRect.x + 3 * r, pen.calculative.worldRect.y + valY);
  path.lineTo(pen.calculative.worldRect.x + r, pen.calculative.worldRect.y + valY);
...
function createdthermometer(path: Path2D, pen: Pen, R: number, r: number) {
  //   path.moveTo(
  //     pen.calculative.worldRect.x + R + r,
  //     pen.calculative.worldRect.y + r
  //   );
  path.arc(pen.calculative.worldRect.x + R, pen.calculative.worldRect.y + r, r, 0, Math.PI, true);
  ...
  path.lineTo(pen.calculative.worldRect.x + R + r, pen.calculative.worldRect.y + r);
}

function getYByValue(pen: Pen, v: number, R: number, r: number, thermometer: any) {
